...
- **MIISOFT + Shelfy — реєстрацію закрито (2026-06-02)**, пивот на живі каталоги. Tier A (free, AI цитує): **Capterra, SaaSHub, G2, AlternativeTo, LawNext Directory**. Tier B (entity-граф): Crunchbase, Wikidata, Product Hunt, GBP, LinkedIn. Tier C (UA-преса): ЮРЛІГА, thepage.ua, ain.ua, dev.ua, mc.today, НААУ ВША. Готовий контент лістингу + деталі — `seo-geo-phase6-offsite.md`.
...
- **Сегменти `/for/*` + `/why` — обговорення «сміттєві сторінки?» (2026-06-02):** друзі власника назвали їх непотрібними. Розбір: `/why` — сильний актив (порівняння = топ-тип для SEO/GEO, AI цитує таблиці), лишаємо однозначно. Сегменти — реальний ризик лише канібалізація `advokat`/`yuryst`/`firma` (один запит, Google ранкує одну, решта простоюють) + `notarius` найслабша (не замінює держреєстри); сайт-штрафу на 34-URL сайті від 4 рукописних сторінок бути не може. Трафік ні ріжуть, ні приносять помітно (новий сайт, мікро-обсяг укр. запитів) — різниця keep/delete зараз = шум; справжня цінність сегментів = конверсія (sales шле таргетоване посилання). **Рішення власника: поки лишити всі 4 без змін.** Перегляд — якщо GSC покаже канібалізацію по сегментах (рекомендація на той випадок: лишити advokat+firma, прибрати yuryst+notarius).
